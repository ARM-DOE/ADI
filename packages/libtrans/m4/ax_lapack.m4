...
# Check for lapacke.h if clapack.h is not found.
AC_CHECK_HEADERS(clapack.h lapacke.h, [found_lapack_header=1 break], [], [])
if test -z "$found_lapack_header"; then
    AC_MSG_ERROR([Could not find LAPACK header file. Searched for clapack.h and lapacke.h])
fi
